...
for (const file of servapps) {
	...
	for (const screenshot of screenshots) {
		servapp.screenshots.push(
			`https://aiola13.github.io/cosmos-servapps-unofficial/servapps/${file}/screenshots/${screenshot}`,
		);
	}
	...
	if (fs.existsSync(`./servapps/${file}/artefacts`)) {
		...
		for (const artefact of artefacts) {
			servapp.artefacts[
				artefact
			] = `https://aiola13.github.io/cosmos-servapps-unofficial/servapps/${file}/artefacts/${artefact}`;
		}
	}
...
for (const servapp of servappsJSON) {
	...
	for (let i = 0; i < servapp.screenshots.length; i++) {
		servapp.screenshots[i] = servapp.screenshots[i].replace(
			'https://aiola13.github.io/cosmos-servapps-unofficial',
			'http://localhost:3000',
		);
	}
